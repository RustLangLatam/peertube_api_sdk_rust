...
pub async fn get_o_auth_client(configuration: &configuration::Configuration, ) -> Result<models::OAuthClient, Error<GetOAuthClientError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v1/oauth-clients/local", local_var_configuration.base_path);
...
pub async fn get_o_auth_token(configuration: &configuration::Configuration, client_id: Option<&str>, client_secret: Option<&str>, grant_type: Option<&str>, username: Option<&str>, password: Option<&str>, refresh_token: Option<&str>) -> Result<models::GetOAuthToken200Response, Error<GetOAuthTokenError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetOAuthTokenError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
...
pub async fn revoke_o_auth_token(configuration: &configuration::Configuration, ) -> Result<(), Error<RevokeOAuthTokenError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/api/v1/users/revoke-token", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RevokeOAuthTokenError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
